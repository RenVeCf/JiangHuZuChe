 ...
 * Time ： 2018/8/26.
 */
public class ModifyUserDataModel<T> extends BaseModel {

    public void getModifyUserData(Context context, TreeMap<String, String> map, boolean isDialog, boolean cancelable,
                                  ObservableTransformer<T, T> transformer, ObserverResponseListener observerListener) {

        //当不需要指定是否由dialog时，可以调用这个方法
        //        subscribe(context, Api.getApiService().SupplementInfo(map), observerListener);
        paramSubscribe(context, Api.getApiService().getModifyUserData(map), observerListener, transformer, isDialog, cancelable);
    }
